...
function buildStandRing(scene, track, crowdMat) {
  ...
  var OFFSETS = { '1': WALL_HALF + 52, '-1': WALL_HALF + 6 };
  ...
  for (var side = 1; side >= -1; side -= 2) {
    ...
    for (var k = 0; k <= n; k += STEP) {
      ...
      for (var t = 0; t < 2; t++) {
        var st = strips[t];
        st.pos.push(p.x + l.x * side * st.near, p.y + st.y0, p.z + l.z * side * st.near);
        st.pos.push(p.x + l.x * side * st.far, p.y + st.y1, p.z + l.z * side * st.far);
        st.uv.push(u, 0, u, 1);
      }
      cols++;
    }
